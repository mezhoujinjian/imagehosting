...
status_t SurfaceFlinger::getDisplayConfigs(const sp<IBinder>& display,
    ...
    for (const auto& hwConfig : getHwComposer().getConfigs(type)) {
        ...
        if (type == DisplayDevice::DISPLAY_PRIMARY) {
            ...
            //feat(Display) 2020/04/03 sys-wangxiaodong fix the issue about dpi in second display Mod-S
            //static const int TV_DENSITY = 213;
            static const int TV_DENSITY = Density::getBuildDensity();
            //feat(Display) 2020/04/03 sys-wangxiaodong fix the issue about dpi in second display Mod-E
...
void SurfaceFlinger::rebuildLayerStacks() {
    ...
    if (CC_UNLIKELY(mVisibleRegionsDirty)) {
        ...
        for (size_t dpy=0 ; dpy<mDisplays.size() ; dpy++) {
             const sp<DisplayDevice>& hw(mDisplays[dpy]);
             if (hw->getCloneLayers().size() > 0) {
                 for (auto& layer : hw->getCloneLayers()) {
                    if (layer->getCloneStatus()) {
                        if (layer->getLayerStack() == 0) {
                            layer->destroyHwcLayer(getSecondDisplayDevice()->getHwcDisplayId());
                        } else if (layer->getLayerStack() == 1) {
                                layer->destroyHwcLayer(getMainDisplayDevice()->getHwcDisplayId());
                        }
                    }
                 }
             }
        ...
        //zhoujinjian AcrossTheScreenInDisId
        /*for (size_t dpy=0 ; dpy < mDisplays.size() ; dpy++) {
             const sp<DisplayDevice>& hw(mDisplays[dpy]);
             if (hw->getAcrossTheScreenLayers().size() > 0) {
                 for (auto& layer : hw->getAcrossTheScreenLayers()) {
                    if (layer->getAcrossTheScreenDisId() >= 0) {
                        if (layer->getAcrossTheScreenDisId() == 0) {
                            layer->destroyHwcLayer(getMainDisplayDevice()->getHwcDisplayId());
                        } else if (layer->getAcrossTheScreenDisId() == 1) {
                            layer->destroyHwcLayer(getSecondDisplayDevice()->getHwcDisplayId());
                        } else if (layer->getAcrossTheScreenDisId() == 2) {
                            layer->destroyHwcLayer(getThirdDisplayDevice()->getHwcDisplayId());
                        }
                    }
                 }
             }
            hw->clearAcrossTheScreenLayers();
        }*/
                ...
                mDrawingState.traverseInZOrder([&](Layer* layer) {
                    ...
                    if (layer->belongsToDisplay(displayDevice->getLayerStack(),
                        ...
                        if (!drawRegion.isEmpty()) {
                            ...
                            if (layer->getCloneStatus()) {
                                if (layer->getLayerStack() == 0) {
                                    getSecondDisplayDevice()->addCloneLayer(layer);
                                } else if (layer->getLayerStack() == 1) {
                                    getMainDisplayDevice()->addCloneLayer(layer);
                                }
                            }
                            if (layer->getAcrossTheScreenDisId() >= 0) {
                               if (layer->getAcrossTheScreenDisId() == 0) {
                                   getMainDisplayDevice()->addAcrossTheScreenLayer(layer);
                               } else if (layer->getAcrossTheScreenDisId() == 1) {
                                   getSecondDisplayDevice()->addAcrossTheScreenLayer(layer);
                               } else if (layer->getAcrossTheScreenDisId() == 2) {
                                   getThirdDisplayDevice()->addAcrossTheScreenLayer(layer);
                               }
                            }
                        } else {
                            ...
                            } else if (layer->getAcrossTheScreenDisId() >= 0) {
                                if (layer->getAcrossTheScreenDisId() == 0) {
                                    layer->destroyHwcLayer(getMainDisplayDevice()->getHwcDisplayId());
                                    hwcLayerDestroyed = layer->destroyHwcLayer(
                                            displayDevice->getHwcDisplayId());
                        ...
                        // zhoujinjian pipTransferDisplay
                        if (layer->getCloneStatus()) {
                            if (layer->getLayerStack() == 0) {
                                layer->destroyHwcLayer(getSecondDisplayDevice()->getHwcDisplayId());
                                hwcLayerDestroyed = layer->destroyHwcLayer(
                        ...
                        //zhoujinjian AcrossTheScreenInDisId
                            ...
                            if (layer->getAcrossTheScreenDisId() == 1) {
                                layer->destroyHwcLayer(getMainDisplayDevice()->getHwcDisplayId());
                                hwcLayerDestroyed = layer->destroyHwcLayer(
                                        displayDevice->getHwcDisplayId());
                            } else if (layer->getAcrossTheScreenDisId() == 2) {
                                    ...
                                    hwcLayerDestroyed = layer->destroyHwcLayer(
                                            displayDevice->getHwcDisplayId());
                            } else if (layer->getAcrossTheScreenDisId() == 3) {
                                    layer->destroyHwcLayer(getThirdDisplayDevice()->getHwcDisplayId());
                                    hwcLayerDestroyed = layer->destroyHwcLayer(
                                            displayDevice->getHwcDisplayId());
                            }
       ...
       //zhoujinjian pipTransferDisplay
        for (size_t dpy=0 ; dpy<mDisplays.size() ; dpy++) {
             ...
             Vector<sp<Layer>> layersNeedingFences= hw->getLayersNeedingFences();
                 if (hw->getCloneLayers().size() > 0) {
                     for (auto& layer : hw->getCloneLayers()) {
                        Region drawRegion(tr.transform(
                                layer->visibleNonTransparentRegion));
        ...
        for (size_t dpy=0 ; dpy<mDisplays.size() ; dpy++) {
            ...
            if (hwcId >= 0) {
                ...
                for (size_t i = 0; i < currentLayers.size(); i++) {
                    ...
                    // }
                    //zhoujinjian AcrossTheScreenInDisId
                    /*if (layer->getName().contains("ThirdStatusBar")
						||layer->getName().contains("SecStatusBar")
						||layer->getName().contains("SecNavigationBar")
						||layer->getName().contains("ThirdNavigationBar")){
                       layer->forceClientComposition(hwcId);
                       continue;
                    }*/
...
void SurfaceFlinger::commitTransaction()
{
    if (!mLayersPendingRemoval.isEmpty()) {
        ...
        for (const auto& l : mLayersPendingRemoval) {
            ...
            ALOGI("fix.tombstones onRemoved name = %s add = %p \n",  l->getName().string(), &l);
...
void SurfaceFlinger::invalidateLayerStack(const sp<const Layer>& layer, const Region& dirty) {
    for (size_t dpy=0 ; dpy<mDisplays.size() ; dpy++) {
        ...
        }else if (layer->getCloneStatus()) {
            hw->dirtyRegion.orSelf(dirty); //zhoujinjian pipTransferDisplay
        }else if (layer->getAcrossTheScreenDisId() >= 0) {
            hw->dirtyRegion.orSelf(dirty); //zhoujinjian AcrossTheScreenInDisId
        }
    }
}
...
{
    // The layer stack of primary display is always 0.
    // If this is a virtual display and it's layer stack is 0 same as primary display,
    // indicate that there has no available content need to show on the virtual display,
    // so unnecessary to render it at here.        Added by YangDan
    if (displayDevice->getDisplayType() >= DisplayDevice::DISPLAY_VIRTUAL) {
        uint32_t layerStack = displayDevice->getLayerStack();
        ALOGI("doDisplayComposition layerStack %d", layerStack);
        if (layerStack == DisplayDevice::NO_LAYER_STACK || layerStack == 0) {
            return;
        }
    }
        ...
        if (!clip.isEmpty()) {
            switch (layer->getCompositionType(hwcId)) {
                ...
                case HWC2::Composition::Client: {
                    //zhoujinjian
                    if(layer->getCloneStatus()){
                        layer->setDisplayDeviceType(displayDevice->getDisplayType());
                    }
                    //zhoujinjian AcrossTheScreenInDisId
                    if(layer->getAcrossTheScreenDisId() >= 0){
                        layer->setAcrossTheScreenDisType(displayDevice->getDisplayType());
                    }
...
status_t SurfaceFlinger::removeLayer(const sp<Layer>& layer, bool topLevelOnly) {
    Mutex::Autolock _l(mStateLock);
    //ALOGI("fix.tombstones removeLayer  %p   %s\n", &layer, layer->getName().string());
    return removeLayerLocked(mStateLock, layer, topLevelOnly);
}

status_t SurfaceFlinger::removeLayerAsync(const sp<Layer>& layer, bool topLevelOnly) {
    Mutex::Autolock _l(mStateLock);
    //ALOGI("fix.tombstones removeLayerAsync %s\n", layer->getName().string());
    class MessageRemoveLayer : public MessageBase {
        SurfaceFlinger* flinger;
		const sp<Layer>& layer;
		bool topLevelOnly;
    public:
        explicit MessageRemoveLayer(SurfaceFlinger* flinger, const sp<Layer>& layer, bool topLevelOnly) 
		: flinger(flinger), layer(layer), topLevelOnly(topLevelOnly) { }
        virtual bool handler() {
            flinger->removeLayer(layer, topLevelOnly);
			//signalTransaction();
            return true;
        }
    };
    sp<MessageBase> msg = new MessageRemoveLayer(this, layer, topLevelOnly);
    postMessageSync(msg);  // we may be called from main thread, use async message
    return NO_ERROR;
}

status_t SurfaceFlinger::removeLayerForAnimBack(const sp<Layer>& layer) {
    Mutex::Autolock _l(mStateLock);
    //ALOGI("fix.tombstones removeLayerForAnimBack %s\n", layer->getName().string());
	//mCurrentState.layersSortedByZ.remove(layer);
	layer->onRemovedCurrentChildren();
...
status_t SurfaceFlinger::removeLayerLocked(const Mutex&, const sp<Layer>& layer,
    ...
    //ALOGI("fix.tombstones removeLayerLocked %s 111111 \n", layer->getName().string());
	//CallStack stack;
	//stack.update();
	//stack.log("SurfaceFlinger::removeLayerLocked");

    const auto& p = layer->getParent();
    ssize_t index;
    if (p != nullptr) {
		//ALOGI("fix.tombstones removeLayerLocked Parent %s 111111 \n", layer->getParent()->getName().string());
        if (topLevelOnly) {
            //ALOGI("fix.tombstones removeLayerLocked %s 222222 \n", layer->getName().string());
			//if(layer->getName().contains("animation background stackId")){
			//	layer->onRemovedZOrderRelativeState();
			//}
            return NO_ERROR;
        }
	...
	//ALOGI("fix.tombstones removeLayerLocked %s 333333 %zd\n", layer->getName().string(), index);
	...
	//ALOGI("fix.tombstones removeLayerLocked %s 444444 \n", layer->getName().string());
...
uint32_t SurfaceFlinger::setClientStateLocked(const ComposerState& composerState) {
    ...
    //zhoujinjian pipTransferDisplay
    if (what & layer_state_t::eCloneFlagsChanged) {
        if (s.mask & layer_state_t::eLayerHidden) {
            if (s.flags & layer_state_t::eLayerHidden) {
                layer->setCloneStatus(false);
                mCurrentState.traverseInZOrder([](Layer* layer) {
                    layer->setCloneStatus(false);
                });

            } else {
                layer->setCloneStatus(true);
                //mCurrentState.traverseInZOrder([](Layer* layer) {
                //    layer->setCloneStatus(true);
                //});
            }
            //ALOGI("pipTransferDisplay setClientStateLocked, clone change, flag=%d, mask=%d", s.flags, s.mask);
            flags |= eTraversalNeeded;
        }
        mVisibleRegionsDirty = true;
    ...
    //zhoujinjian AcrossTheScreenInDisId
    if (what & layer_state_t::eAcrossTheScreenInDisId) {
        layer->setAcrossTheScreenDisId(s.acrossTheScreenInDisId);
        flags |= eTraversalNeeded;
        mVisibleRegionsDirty = true;
    }
...
void SurfaceFlinger::setDestroyStateLocked(const ComposerState& composerState) {
    ...
    //zhoujinjian remove it,it case tombstones
...
status_t SurfaceFlinger::onLayerRemoved(const sp<Client>& client, const sp<IBinder>& handle)
{
    ...
    if (l != nullptr) {
        ...
        ALOGE("onLayerRemoved");
	    //if(l->getName().contains("Stack=")){
			//err = removeLayerForAnimBack(l);
		//    err = removeLayer(l, false);
		//} else {
            err = removeLayer(l, false);
		//}
...
status_t SurfaceFlinger::tellSFPendingDestroy(const sp<Client>& client, const sp<IBinder>& handle)
{
    // called by a client when it wants to remove a Layer
    //status_t err = NO_ERROR;
	ALOGE("tellSFPendingDestroy");
    Mutex::Autolock _l(mStateLock);
...
status_t SurfaceFlinger::onLayerPendingDestroy(const wp<Layer>& layer)
{
    Mutex::Autolock _l(mStateLock);
...
status_t SurfaceFlinger::onLayerDestroyed(const wp<Layer>& layer)
{
	...
	ALOGE("onLayerDestroyed");
    // If we have a parent, then we can continue to live as long as it does.
	if(l->getName().contains("Stack=")){
		//return removeLayer(l, true);
		return removeLayer(l,true);
		//mLayersPendingRemoval.add(l);
		//mLayersRemoved = true;
		//return NO_ERROR;
	} else {
        return removeLayer(l, true);
